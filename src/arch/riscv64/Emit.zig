...
fn mirRType(emit: *Emit, inst: Mir.Inst.Index) !void {
    ...
    switch (instruction) {
        ...
        .snez => |r_type| try emit.writeInstruction(Instruction.sltu(r_type.rd, .zero, r_type.rs1)),
...
fn mirIType(emit: *Emit, inst: Mir.Inst.Index) !void {
    ...
    switch (instruction) {
        ...
        .csrrsi => |i_type| try emit.writeInstruction(Instruction.csrrsi(i_type.rd, i_type.rs1, i_type.imm12)),
        ...
        .mv => |rr_type| try emit.writeInstruction(Instruction.addi(rr_type.rd, rr_type.rs1, 0)),
        ...
        .seqi => |i_type| {
            try emit.writeInstruction(Instruction.addi(i_type.rd, i_type.rs1, -i_type.imm12));
            try emit.writeInstruction(Instruction.sltiu(i_type.rd, i_type.rs1, 1));
        },
        .seqz => |rr_type| try emit.writeInstruction(Instruction.sltiu(rr_type.rd, rr_type.rs1, 1)),
        .sgti => |i_type| {
            try emit.writeInstruction(Instruction.slti(i_type.rd, i_type.rs1, i_type.imm12 + 1));
            try emit.writeInstruction(Instruction.xori(i_type.rd, i_type.rd, 1));
        },
        .sgtui => |i_type| {
            try emit.writeInstruction(Instruction.sltiu(i_type.rd, i_type.rs1, @as(u12, @intCast(i_type.imm12)) + 1));
            try emit.writeInstruction(Instruction.xori(i_type.rd, i_type.rd, 1));
        },
        .sgtei => |i_type| {
            try emit.writeInstruction(Instruction.slti(i_type.rd, i_type.rs1, i_type.imm12));
            try emit.writeInstruction(Instruction.xori(i_type.rd, i_type.rd, 1));
        },
        .sgteui => |i_type| {
            try emit.writeInstruction(Instruction.sltiu(i_type.rd, i_type.rs1, @as(u12, @bitCast(i_type.imm12))));
            try emit.writeInstruction(Instruction.xori(i_type.rd, i_type.rd, 1));
        },
        ...
        .sltui => |i_type| try emit.writeInstruction(Instruction.sltiu(i_type.rd, i_type.rs1, @as(u12, @bitCast(i_type.imm12)))),
        .sltei => |i_type| try emit.writeInstruction(Instruction.slti(i_type.rd, i_type.rs1, i_type.imm12 + 1)),
        .slteui => |i_type| try emit.writeInstruction(Instruction.sltiu(i_type.rd, i_type.rs1, @as(u12, @bitCast(i_type.imm12)) + 1)),
        .sneqi => |i_type| {
            try emit.writeInstruction(Instruction.addi(i_type.rd, i_type.rs1, -i_type.imm12));
            try emit.writeInstruction(Instruction.sltu(i_type.rd, .zero, i_type.rs1));
        },
